...
public class UserEntityService {
	...
	private static final Logger LOG = LoggerFactory
			.getLogger(UserEntityService.class);
	...
	public UserEntity getUser() {
		...
		if (principal instanceof LdapUserDetailsImpl) {
			...
			UserEntity userEntity = userEntityRepository
					.findByUsername(ldapUser.getUsername());
			if (userEntity == null) {
				createUser(ldapUser.getUsername(), "", "",
						sha256("" + System.currentTimeMillis()), false, null);
			}

			// sync user with ldap
			userEntity = userEntityRepository.findByUsername(ldapUser
					.getUsername());
			DirContextAdapter dca = (DirContextAdapter) ldap.lookup(ldapUser
					.getDn());
			...
			if (userEntity.getEmail() == null
					|| !userEntity.getEmail().equals(mail)) {
				userEntity.setEmail(mail);
				changes = true;
			}
			if (userEntity.getFullname() == null
					|| !userEntity.getFullname().equals(fullname)) {
			...
			UserEntity user = userEntityRepository.findByUsername(userDetails
					.getUsername());

			if (user == null) {
				LOG.warn("User from session not found. username={}",
	...
	public boolean userExists(String username) {
		return username != null
				&& userEntityRepository.findByUsername(username) != null;
	}
	 ...
	 */
	@Transactional
	public void createUser(String username, String email, String fullname,
		...
		LOG.info("User '{}' has created a new user '{}'", getUser()
				.getUsername(), username);
	}

	@Transactional
	public void editUser(String username, String email, String fullname,
			String password, boolean admin, String group) {
		if (!isAuthenticated()) {
			return;
		}
		...
		userEntityRepository.save(user);
		LOG.info("User '{}' has edited the user '{}'", getUser().getUsername(),
				username);
	}
	...
	public void createAdminAccount() {
		...
		if (adminUser == null) {
			...
			adminUser
					.setPasswordHash("8c6976e5b5410415bde908bd4dee15dfb167a9c873fc4bb8a81f6f2ab448a918"); // sha256("admin")
	...
	public void setUserEntityRepository(
