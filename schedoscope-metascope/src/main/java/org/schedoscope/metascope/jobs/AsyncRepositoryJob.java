...
public abstract class AsyncRepositoryJob implements Runnable {
  ...
  public AsyncRepositoryJob(ViewEntity viewEntity, ViewEntityRepository viewEntityRepository, 
      JobMetadataEntityRepository jobMetadataEntityRepository, MetascopeConfig config, 
      String jobMetadataKey, String jobMetadataField) {
  ...
  private void postJob() {
    JobMetadataEntity jobMetadata = jobMetadataEntityRepository.findOne(new JobMetadataEntityKey(jobMetadataKey, jobMetadataField));
    if (jobMetadata == null) {
      return;
    }
