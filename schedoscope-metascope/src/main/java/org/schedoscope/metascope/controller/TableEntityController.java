...
public class TableEntityController extends ViewController {
	...
	private static final Logger LOG = LoggerFactory
			.getLogger(TableEntityController.class);
	...
	public ModelAndView getTable(HttpServletRequest request) {
		...
		Map<String, CategoryMap> tableTaxonomies = tableEntityService
				.getTableTaxonomies(tableEntity);
		...
		ViewEntity selectedViewEntity = tableEntityService.runDataDistribution(
				tableEntity, selectedPartition, partitionCount);
		...
		if (transitive) {
			...
			List<TableDependencyEntity> transitiveDependencies = tableEntityService
					.getTransitiveDependencies(tableEntity);
			List<TableDependencyEntity> transitiveSuccessors = tableEntityService
			...
			List<TableDependencyEntity> successors = tableEntityService
					.getSuccessors(tableEntity);
			mav.addObject("successors", successors);
		}

		/*
		 * if table is partitioned, get the first parameter (see 'Data
		...
		/*
		 * local parameter indicates if the css and javascript should be
		 * containted in the html. this can be useful when displaying views in
	...
	public String changeOwner(HttpServletRequest request,
			RedirectAttributes redirAttr, String fqdn, String person) {
	...
	public String admin(HttpServletRequest request, String fqdn,
			String dataTimestampField, String dataTimestampFieldFormat) {
		tableEntityService.setTimestampField(fqdn, dataTimestampField,
				dataTimestampFieldFormat);
	 ...
	 * 
	 * @param request
	 *            the HTTPServletRequest from the client containing view
	 *            parameter values
	...
	public String requestDatadist(HttpServletRequest request,
	...
	@ResponseBody
	public String getSample(String fqdn,
		...
		Future<HiveQueryResult> future = tableEntityService.getSample(fqdn,
				params);
	...
	public ModelAndView getViews(String fqdn, int partitionPage) {
		...
		TableEntity tableEntity = tableEntityService.findByFqdn(fqdn);
		Page<ViewEntity> views = tableEntityService.getRequestedViewPage(fqdn,
	...
	public ModelAndView getParameterValues(HttpServletRequest request) {
		...
		Set<String> parameterValues = tableEntityService.getParameterValues(
				tableEntity, urlPrefix, parameter);
		mav.addObject("table", tableEntity);
