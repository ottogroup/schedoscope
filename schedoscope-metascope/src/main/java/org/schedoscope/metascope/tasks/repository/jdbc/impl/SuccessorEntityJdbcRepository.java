...
public class SuccessorEntityJdbcRepository implements
		JdbcRepository<SuccessorEntity, SuccessorEntityKey> {

	private static final Logger LOG = LoggerFactory
			.getLogger(SuccessorEntityJdbcRepository.class);
		...
		try {
			stmt = connection
					.prepareStatement("select "
							+ JDBCUtil
									.getDatabaseColumnsForClass(SuccessorEntity.class)
							+ " from successor_entity "
							+ "where url_path = ? and successor_url_path = ? and successor_fqdn = ?");
			...
			if (rs.next()) {
				...
				successorEntity.setUrlPath(rs
						.getString(SuccessorEntity.URL_PATH));
				successorEntity.setSuccessorUrlPath(rs
						.getString(SuccessorEntity.SUCCESSOR_URL_PATH));
				successorEntity.setSuccessorFqdn(rs
						.getString(SuccessorEntity.SUCCESSOR_FQDN));
				successorEntity.setInternalViewId(rs
						.getString(SuccessorEntity.INTERNAL_VIEW_ID));
			}
	...
	public SuccessorEntity get(Connection connection,
		...
		try {
			stmt = connection
					.prepareStatement("select * from successor_entity where "
							+ SuccessorEntity.URL_PATH + " = ? and "
							+ SuccessorEntity.SUCCESSOR_URL_PATH + " = ? and "
							+ SuccessorEntity.SUCCESSOR_FQDN + " = ?");
	...
	public void save(Connection connection, SuccessorEntity successorEntity) {
		String insertSuccessorSql = "insert into successor_entity ("
				+ JDBCUtil.getDatabaseColumnsForClass(SuccessorEntity.class)
				+ ") values ("
	...
	public void update(Connection connection, SuccessorEntity successorEntity) {
		String updateSuccessorSql = "update successor_entity set "
				+ JDBCUtil.getSetExpressionForClass(SuccessorEntity.class)
				+ " where " + SuccessorEntity.URL_PATH + " = ? and "
				+ SuccessorEntity.SUCCESSOR_URL_PATH + " = ? and "
				+ SuccessorEntity.SUCCESSOR_FQDN + " = ?";
		PreparedStatement stmt = null;
		try {
