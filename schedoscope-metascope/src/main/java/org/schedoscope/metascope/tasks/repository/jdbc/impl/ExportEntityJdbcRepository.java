...
public class ExportEntityJdbcRepository implements
		JdbcRepository<ExportEntity, String> {

	private static final Logger LOG = LoggerFactory
			.getLogger(ExportEntityJdbcRepository.class);
	...
	public void save(Connection connection, ExportEntity exportEntity) {
		String insertExportSql = "insert into export_entity ("
				+ JDBCUtil.getDatabaseColumnsForClass(ExportEntity.class)
				+ ") values ("
				+ JDBCUtil.getValuesCountForClass(ExportEntity.class) + ")";
		...
		try {
			...
			for (Entry<String, String> e : exportEntity.getProperties()
					.entrySet()) {
				stmt.setString(1, exportEntity.getExportId());
