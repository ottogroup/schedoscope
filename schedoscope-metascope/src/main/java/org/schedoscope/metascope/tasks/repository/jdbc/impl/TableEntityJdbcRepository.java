...
public class TableEntityJdbcRepository implements
		JdbcRepository<TableEntity, String> {

	private static final Logger LOG = LoggerFactory
			.getLogger(TableEntityJdbcRepository.class);
	...
	public TableEntity get(Connection connection, String key) {
		...
		try {
			stmt = connection.prepareStatement("select "
					+ JDBCUtil.getDatabaseColumnsForClass(TableEntity.class)
			...
			if (rs.next()) {
				...
				tableEntity.setDatabaseName(rs
						.getString(TableEntity.DATABASE_NAME));
				tableEntity.setUrlPathPrefix(rs
						.getString(TableEntity.URL_PATH_PREFIX));
				tableEntity.setExternalTable(rs
						.getBoolean(TableEntity.EXTERNAL_TABLE));
				tableEntity.setTableDescription(rs
						.getString(TableEntity.TABLE_DESCRIPTION));
				tableEntity.setStorageFormat(rs
						.getString(TableEntity.STORAGE_FORMAT));
				tableEntity.setInputFormat(rs
						.getString(TableEntity.INPUT_FORMAT));
				tableEntity.setOutputFormat(rs
						.getString(TableEntity.OUTPUT_FORMAT));
				tableEntity.setMaterializeOnce(rs
						.getBoolean(TableEntity.MATERIALIZE_ONCE));
				tableEntity.setCreatedAt(rs.getLong(TableEntity.CREATED_AT));
				tableEntity.setStatus(rs.getString(TableEntity.STATUS));
				tableEntity
						.setTableOwner(rs.getString(TableEntity.TABLE_OWNER));
				tableEntity.setDataPath(rs.getString(TableEntity.DATA_PATH));
				tableEntity.setDataSize(rs.getLong(TableEntity.DATA_SIZE));
				tableEntity.setPermissions(rs
						.getString(TableEntity.PERMISSIONS));
				tableEntity.setRowcount(rs.getLong(TableEntity.ROWCOUNT));
				tableEntity.setTransformationType(rs
						.getString(TableEntity.TRANSFORMATION_TYPE));
				tableEntity.setLastData(rs.getString(TableEntity.LAST_DATA));
				tableEntity.setTimestampField(rs
						.getString(TableEntity.TIMESTAMP_FIELD));
				tableEntity.setTimestampFieldFormat(rs
						.getString(TableEntity.TIMESTAMP_FIELD_FORMAT));
				tableEntity.setLastChange(rs.getLong(TableEntity.LAST_CHANGE));
				tableEntity.setLastPartitionCreated(rs
						.getLong(TableEntity.LAST_PARTITION_CREATED));
				tableEntity.setLastSchemaChange(rs
						.getLong(TableEntity.LAST_SCHEMA_CHANGE));
				tableEntity.setLastTransformation(rs
						.getLong(TableEntity.LAST_TRANSFORMATION_TIMESTAMP));
			}
	...
	public void update(Connection connection, TableEntity tableEntity) {
		String updateTableSql = "update table_entity set "
				+ JDBCUtil.getSetExpressionForClass(TableEntity.class)
	...
	public void updateStatus(Connection connection, TableEntity tableEntity,
			Long end) {
		String updateTableStatusSql = "update table_entity set "
				+ TableEntity.STATUS + " = ?";
		if (end != null) {
			updateTableStatusSql += ", "
					+ TableEntity.LAST_TRANSFORMATION_TIMESTAMP + " = ?";
		}
