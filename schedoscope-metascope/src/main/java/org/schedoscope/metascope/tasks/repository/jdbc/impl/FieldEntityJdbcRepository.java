...
public class FieldEntityJdbcRepository implements JdbcRepository<FieldEntity, FieldEntityKey> {

    private static final Logger LOG = LoggerFactory.getLogger(FieldEntityJdbcRepository.class);
    ...
    public void save(Connection connection, FieldEntity fieldEntity) {
        String insertFieldSql = "insert into field_entity (" + JDBCUtil.getDatabaseColumnsForClass(FieldEntity.class)
                + ") values (" + JDBCUtil.getValuesCountForClass(FieldEntity.class) + ")";
        PreparedStatement stmt = null;
        try {
