...
public class ViewDependencyEntityJdbcRepository implements
		JdbcRepository<ViewDependencyEntity, DependencyEntityKey> {

	private static final Logger LOG = LoggerFactory
			.getLogger(ViewDependencyEntityJdbcRepository.class);

	@Override
	public ViewDependencyEntity get(Connection connection,
			DependencyEntityKey key) {
		...
		try {
			...
			if (rs.next()) {
				...
				dependencyEntity.setUrlPath(rs
						.getString(ViewDependencyEntity.URL_PATH));
				dependencyEntity.setDependencyUrlPath(rs
						.getString(ViewDependencyEntity.DEPENDENCY_URL_PATH));
				dependencyEntity.setDependencyFqdn(rs
						.getString(ViewDependencyEntity.DEPENDENCY_FQDN));
				dependencyEntity.setInternalViewId(rs
						.getString(ViewDependencyEntity.INTERNAL_VIEW_ID));
			}
	...
	public void update(Connection connection,
		...
		String updateDependencySql = "update view_dependency_entity set "
				+ JDBCUtil.getSetExpressionForClass(ViewDependencyEntity.class)
				+ " where " + ViewDependencyEntity.URL_PATH + " = ? and "
				+ ViewDependencyEntity.DEPENDENCY_URL_PATH + " = ? and "
				+ ViewDependencyEntity.DEPENDENCY_FQDN + " = ?";
		PreparedStatement stmt = null;
		try {
