...
public class TransformationEntityJdbcRepository implements
	...
	private static final Logger LOG = LoggerFactory
			.getLogger(TransformationEntityJdbcRepository.class);

	@Override
	public TransformationEntity get(Connection connection,
			TransformationEntityKey key) {
		...
		try {
			stmt = connection
					.prepareStatement("select "
							+ JDBCUtil
									.getDatabaseColumnsForClass(TransformationEntity.class)
			...
			if (rs.next()) {
				...
				transformationEntity.setFqdn(rs
						.getString(TransformationEntity.FQDN));
				transformationEntity.setTransformationKey(rs
						.getString(TransformationEntity.KEY));
				transformationEntity.setTransformationValue(rs
						.getString(TransformationEntity.VALUE));
			}
	...
	public void update(Connection connection, TransformationEntity te) {
		String updateTeSql = "update transformation_entity set "
				+ JDBCUtil.getSetExpressionForClass(TransformationEntity.class)
				+ " where " + TransformationEntity.FQDN + " = ? and "
				+ TransformationEntity.KEY + "= ?";
		PreparedStatement stmt = null;
		try {
