...
public class TableDependencyEntityJdbcRepository implements
  ...
  private static final Logger LOG = LoggerFactory.getLogger(TableDependencyEntityJdbcRepository.class);

  @Override
  public TableDependencyEntity get(Connection connection, TableDependencyEntityKey key) {
  ...
  public void update(Connection connection, TableDependencyEntity dependency) {
    String updateDependencySql = "update table_dependency_entity set "
        + JDBCUtil.getSetExpressionForClass(TableDependencyEntity.class) + " where " + TableDependencyEntity.FQDN
        + " = ? and " + TableDependencyEntity.DEPENDENCY_FQDN + " = ?";
    PreparedStatement stmt = null;
    try {
