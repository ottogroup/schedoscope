...
public class ViewEntityJdbcRepository implements
		JdbcRepository<ViewEntity, String> {

	private static final Logger LOG = LoggerFactory
			.getLogger(ViewEntityJdbcRepository.class);
	...
	public ViewEntity get(Connection connection, String urlPath) {
		...
		try {
			stmt = connection.prepareStatement("select "
					+ JDBCUtil.getDatabaseColumnsForClass(ViewEntity.class)
			...
			if (rs.next()) {
				...
				viewEntity.setParameterString(rs
						.getString(ViewEntity.PARAMETERSTRING));
				viewEntity.setInternalViewId(rs
						.getString(ViewEntity.INTERNAL_VIEW_ID));
				viewEntity.setTransformationStart(rs
						.getLong(ViewEntity.TRANSFORMATION_START));
				viewEntity.setTransformationEnd(rs
						.getLong(ViewEntity.TRANSFORMATION_END));
				viewEntity.setCreatedAt(rs.getLong(ViewEntity.CREATED_AT));
				viewEntity.setRowJobFinished(rs
						.getBoolean(ViewEntity.ROW_JOB_FINISHED));
	...
	public void save(Connection connection, ViewEntity viewEntity) {
		String insertViewSql = "insert into view_entity ("
				+ JDBCUtil.getDatabaseColumnsForClass(ViewEntity.class)
				+ ") values ("
				+ JDBCUtil.getValuesCountForClass(ViewEntity.class) + ")";
		PreparedStatement stmt = null;
		try {
	...
	public void update(Connection connection, ViewEntity viewEntity) {
		String updateViewSql = "update view_entity set "
				+ JDBCUtil.getSetExpressionForClass(ViewEntity.class)
	...
	public void updateStatus(Connection connection, ViewEntity viewEntity,
			Long start, Long end) {
		String updateViewStatusSql = "update view_entity set "
				+ ViewEntity.STATUS + " = ?";
		if (start != null) {
