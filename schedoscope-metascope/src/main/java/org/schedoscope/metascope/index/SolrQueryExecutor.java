...
public class SolrQueryExecutor {
  ...
  public SolrQueryExecutor(SolrClient solrClient, TableEntityService tableEntityService,
      ViewEntityService viewEntityService, FieldEntityService fieldEntityService) {
  ...
  /**
   * Perform a query on the metascope solr index. Builds up the query
   * string from the given parameters and sends request to solr server
   * 
   * @param params
    ...
    for (SolrDocument solrDocument : list) {
      ...
      if (typeFilterValue.equalsIgnoreCase(TYPE_PARTITION)) {
        if (!searchQueryValue.equals("*")) {
          resultEntities.add(new SolrQueryResultEntity(viewEntityService.findByUrlPath(id), queryResponse
        ...
        if (!searchQueryValue.equals("*")) {
          resultEntities.add(new SolrQueryResultEntity(tableEntityService.findByFqdn(id), queryResponse
              .getHighlighting().get(id)));
        } else {
