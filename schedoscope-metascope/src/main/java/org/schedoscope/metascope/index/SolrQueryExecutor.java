...
import org.schedoscope.metascope.index.model.SolrFacet;
import org.schedoscope.metascope.index.model.SolrFacetQuery;
import org.schedoscope.metascope.index.model.SolrHourRange;
import org.schedoscope.metascope.index.model.SolrQueryParameter;
...
import org.schedoscope.metascope.index.model.SolrQueryResultEntity;
...
public class SolrQueryExecutor {
	...
	public SolrQueryExecutor(SolrClient solrClient,
			TableEntityService tableEntityService,
			ViewEntityService viewEntityService,
			FieldEntityService fieldEntityService) {
		this.solrClient = solrClient;
		this.tableEntityService = tableEntityService;
		this.viewEntityService = viewEntityService;
		...
		this.facetFields.add(new SolrQueryParameter(FILTER_DATABASE,
				DATABASE_NAME, true, FilterType.OR, FacetSort.COUNT));
		this.facetFields.add(new SolrQueryParameter(FILTER_TABLE, TABLE_NAME,
				false, FilterType.AND, FacetSort.COUNT));
		this.facetFields.add(new SolrQueryParameter(FILTER_STATUS, STATUS,
				true, FilterType.EXCLUSIVE, FacetSort.COUNT));
		this.facetFields.add(new SolrQueryParameter(FILTER_TRANSFORMATION,
				TRANSFORMATION, true, FilterType.EXCLUSIVE, FacetSort.COUNT));
		this.facetFields.add(new SolrQueryParameter(FILTER_EXPORT, EXPORTS,
				true, FilterType.EXCLUSIVE, FacetSort.COUNT));
		this.facetFields.add(new SolrQueryParameter(FILTER_STORAGEFORMAT,
				STORAGE_FORMAT, true, FilterType.EXCLUSIVE, FacetSort.COUNT));
		this.facetFields.add(new SolrQueryParameter(FILTER_TAXONOMY,
				TAXONOMIES, false, FilterType.AND, FacetSort.COUNT));
		this.facetFields.add(new SolrQueryParameter(FILTER_CATEGORIES,
				CATEGORIES, false, FilterType.AND, FacetSort.COUNT));
		this.facetFields.add(new SolrQueryParameter(FILTER_CATEGORY_OBJECT,
				CATEGORY_OBJECTS, false, FilterType.AND, FacetSort.COUNT));
		this.facetFields.add(new SolrQueryParameter(FILTER_TAGS, TAGS, false,
				FilterType.AND, FacetSort.COUNT));
		this.facetQueries.add(new SolrFacetQuery("Creation Time", CREATED_AT)
				.withRange(new SolrHourRange("last 60 minutes", 1))
				.withRange(new SolrHourRange("last 24 hours", 24))
				.withRange(new SolrHourRange("last 7 days", 168))
				.withRange(new SolrHourRange("last month", 672))
				.withRange(new SolrHourRange("last year", 8760))
				.withRange(new SolrHourRange("older", Long.MAX_VALUE)));
		this.facetQueries.add(new SolrFacetQuery("Transformation Time",
	...
	public List<String> suggest(String userInput) {
		...
		if (queryResponse != null) {
			SuggesterResponse suggestorRespone = queryResponse
					.getSuggesterResponse();
			if (suggestorRespone != null) {
				Map<String, List<Suggestion>> suggestorResponeMap = suggestorRespone
						.getSuggestions();
				for (Entry<String, List<Suggestion>> e : suggestorResponeMap
						.entrySet()) {
	...
	public SolrQueryResult query(Map<String, String> params) {
		...
		/*
		 * list of filter which have been set by the user (key = filtername,
		 * value = list of selections)
		 */
		Map<String, List<String>> activeFilterValues = new HashMap<String, List<String>>();

		/*
		 * determine on which type is searched for (either 'Table' or
		 * 'Partition'
		 */
		String typeFilterValue = params.get(TYPE);
		filter.add(new SolrQueryParameter(FILTER_TYPE, TYPE,
				FilterType.EXCLUSIVE, FacetSort.COUNT));
		if (typeFilterValue != null
				&& typeFilterValue.equalsIgnoreCase(TYPE_PARTITION)) {
			typeFilterValue = TYPE_PARTITION;
			for (String parameter : fieldEntityService.findDistinctParameters()) {
				partitionFilter.add(new SolrQueryParameter("Parameter: "
						+ parameter, parameter + "_s", FilterType.AND,
						FacetSort.INDEX));
			}
			for (SolrQueryParameter pFilter : partitionFilter) {
				...
				if (filterValue != null && !filterValue.isEmpty()) {
					query.addFilterQuery(pFilter.getName() + ":" + "("
							+ filterValue.replace(",", " AND ") + ")");
				}
		...
		query.addFilterQuery("{!tag=" + TYPE + "}" + TYPE + ":"
				+ typeFilterValue);
			...
			if (queryFilter.isExcludeFromFacet()) {
				if (value != null) {
					...
					for (int i = 0; i < multipleFilter.length; i++) {
						String filterValue = cleanseValue(multipleFilter[i])
								.replace(" ", "*");
						value += "(" + filterValue + ")";
						if (i < multipleFilter.length - 1) {
							value += " "
									+ getOperator(queryFilter.getFilterType())
									+ " ";
						}
					}
					value += ")";
				}
				filterQuery = "{!tag=" + queryFilter.getName() + "}"
						+ queryFilter.getName() + ":" + value;
				facetField = "{!ex=" + queryFilter.getName() + "}"
						+ queryFilter.getName();
			} else {
				if (value != null) {
					String[] multipleFilter = value.split(",");
		...
		for (SolrFacetQuery solrFacetQuery : facetQueries) {
			for (SolrHourRange range : solrFacetQuery.getRanges()) {
				long from = range.getFrom() == Long.MAX_VALUE ? 0 : now
						- (range.getFrom() * 3600);
				String facetQueryString = solrFacetQuery.getName() + ":["
						+ from + " TO " + now + "]";
				query.addFacetQuery("{!ex=" + solrFacetQuery.getName() + "}"
						+ facetQueryString);
				facetQueryMap.put(solrFacetQuery.getName() + range.getName(),
						facetQueryString);
			}
		...
		for (SolrDocument solrDocument : list) {
			...
			if (typeFilterValue.equalsIgnoreCase(TYPE_PARTITION)) {
				if (!searchQueryValue.equals("*")) {
					resultEntities.add(new SolrQueryResultEntity(
							viewEntityService.findByUrlPath(id), queryResponse
									.getHighlighting().get(id)));
				} else {
					resultEntities.add(new SolrQueryResultEntity(
							viewEntityService.findByUrlPath(id)));
				}
				resultType = TYPE_PARTITION;
			} else if (typeFilterValue.equalsIgnoreCase(TYPE_TABLE)) {
				if (!searchQueryValue.equals("*")) {
					resultEntities.add(new SolrQueryResultEntity(
							tableEntityService.findByFqdn(id), queryResponse
									.getHighlighting().get(id)));
				} else {
					resultEntities.add(new SolrQueryResultEntity(
							tableEntityService.findByFqdn(id)));
				}
			}
		}
		...
		for (SolrFacetQuery solrFacetQuery : facetQueries) {
			filter.add(new SolrQueryParameter(solrFacetQuery.getDisplayName(),
					solrFacetQuery.getName(), FilterType.EXCLUSIVE));
			List<SolrFacet> values = new ArrayList<SolrFacet>();
			for (SolrHourRange range : solrFacetQuery.getRanges()) {
				long facetQueryCount = getFacetQueryCount(
						queryResponse,
						"{!ex="
								+ solrFacetQuery.getName()
								+ "}"
								+ facetQueryMap.get(solrFacetQuery.getName()
										+ range.getName()));
		...
		SolrQueryResult result = new SolrQueryResult()
				.withResultEntities(resultEntities)
				.withResultType(resultType)
				.withFilters(filter)
				.withFacetValues(facetValues)
				.withActiveFilterValues(activeFilterValues)
				.withSearchQuery(searchQueryValue)
				.withPage(page)
				.withElements(elements)
				.withTotalPages(
						(int) Math.ceil(((double) list.getNumFound())
								/ elements))
	...
	private long getFacetQueryCount(QueryResponse queryResponse, String key) {
		for (Entry<String, Integer> e : queryResponse.getFacetQuery()
				.entrySet()) {
	...
	private void addToActiveFilterValues(
			Map<String, List<String>> activeFilters,
			Map<String, String> params, List<SolrQueryParameter> filter) {
		for (SolrQueryParameter f : filter) {
			String paramValue = params.get(f.getName()) != null ? params.get(f
					.getName()) : "";
