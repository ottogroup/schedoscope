...
public class SolrFacadeTest extends SpringTest {
	...
	public void solrFacade_01_createTableEntity() {
		SolrInputDocument tableDocument = solr
				.getById(getTestTable().getFqdn());
	...
	public void solrFacade_02_updateTableEntity() {
		...
		String transformation = (String) tableDocument
				.getFieldValue(SolrUpdateHandler.TRANSFORMATION);
		...
		String newTransformation = (String) tableDocument
				.getFieldValue(SolrUpdateHandler.TRANSFORMATION);
	...
	public void solrFacade_03_updateTableEntityAsync() {
		...
		Future<Void> tableEntityAsync = solr.updateTableEntityAsync(
				tableEntity, true);
	...
	public void solrFacade_05_updateTableStatus() {
		...
		String status = (String) tableDocument
				.getFieldValue(SolrUpdateHandler.STATUS);
		Object transformationEnd = tableDocument
				.getFieldValue(SolrUpdateHandler.TRANSFORMATIONTIMESTAMP);
		...
		long newTransformationEnd = (long) tableDocument
				.getFieldValue(SolrUpdateHandler.TRANSFORMATIONTIMESTAMP);
	...
	public void solrFacade_06_createViewEntity() {
		SolrInputDocument viewDocument = solr.getById(getTestView()
				.getUrlPath());
	...
	public void solrFacade_07_updateViewEntity() {
		...
		String status = (String) viewDocument
				.getFieldValue(SolrUpdateHandler.STATUS);
		...
		String newStatus = (String) viewDocument
				.getFieldValue(SolrUpdateHandler.STATUS);
	...
	public void solrFacade_08_updateViewEntityAsync() {
		...
		Future<Void> viewEntityAsync = solr.updateViewEntityAsync(viewEntity,
				true);
	...
	public void solrFacade_09_updateViewStatus() {
		...
		String status = (String) viewDocument
				.getFieldValue(SolrUpdateHandler.STATUS);
		long transformationEnd = (long) viewDocument
				.getFieldValue(SolrUpdateHandler.TRANSFORMATIONTIMESTAMP);
		long createdAt = (long) viewDocument
				.getFieldValue(SolrUpdateHandler.CREATED_AT);
		...
		long newTransformationEnd = (long) viewDocument
				.getFieldValue(SolrUpdateHandler.TRANSFORMATIONTIMESTAMP);
		long newCreatedAt = (long) viewDocument
				.getFieldValue(SolrUpdateHandler.CREATED_AT);
	...
	public void solrFacade_10_testQueryPlain() {
		...
		assertEquals(queryResult.getElements(),
				URLUtil.PAGINATION_ELEMENTS_DEFAULT);
		...
		for (Entry<String, List<String>> e : queryResult
				.getActiveFilterValues().entrySet()) {
	...
	public void solrFacade_11_testQueryPartitions() {
		...
		assertEquals(queryResult.getElements(),
				URLUtil.PAGINATION_ELEMENTS_DEFAULT);
		assertEquals(queryResult.getResultType(),
				SolrQueryExecutor.TYPE_PARTITION);
		assertEquals(queryResult.getStart(), 1);
		assertEquals(queryResult.getTotalResults(), 1);
		for (Entry<String, List<String>> e : queryResult
				.getActiveFilterValues().entrySet()) {
	...
	public void solrFacade_13_testQueryWithDatabaseFilter() {
		...
		parameter.put(SolrQueryExecutor.DATABASE_NAME, getTestTable()
				.getDatabaseName());
