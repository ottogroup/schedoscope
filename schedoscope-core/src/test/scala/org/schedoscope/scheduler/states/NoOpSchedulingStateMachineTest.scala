...
class NoOpSchedulingStateMachineTest extends FlatSpec with Matchers {
  ...
  "A NoOp leaf view in CreatedByViewManagerState with no _SUCCESS flag" should "transition to NoData upon materialize" in new NoOpLeafView {
    ...
    viewUnderTest.materialize(startState, dependentView, successFlagExists = false) match {
      case ResultingViewSchedulingState(NoData(v, viewTransformationChecksum), _) => v shouldBe viewUnderTest
      case _ => fail()
    }
  }
  ...
  "A NoOp leaf view in NoDataState with existing _SUCCESS flag" should "transition to Materialized upon materialize" in new NoOpLeafView {
    val startState = NoData(viewUnderTest, viewTransformationChecksum)
