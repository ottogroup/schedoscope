...
class NoOpLeafViewSchedulingStateMachineTest extends FlatSpec with Matchers {
  ...
  trait NoOpLeafView {
    val stateMachine = new NoOpLeafViewSchedulingStateMachine()
  ...
  it should "request writing of new transformation checksum" in new NoOpLeafView {
    ...
    stateMachine.materialize(startState, dependentView, successFlagExists = true) match {
  ...
  "A NoOp leaf view in CreatedByViewManager state with no _SUCCESS flag" should "transition to NoData upon materialize" in new NoOpLeafView {
    ...
    stateMachine.materialize(startState, dependentView, successFlagExists = false) match {
  ...
  it should "request writing of new transformation checksum if materialization mode is RESET_TRANSFORMATION_CHECKSUMS" in new NoOpLeafView {
    val startState = Materialized(viewUnderTest, viewTransformationChecksum, 10, withErrors = false, incomplete = false)

    stateMachine.materialize(startState, dependentView, successFlagExists = true, materializationMode = RESET_TRANSFORMATION_CHECKSUMS) match {
      case ResultingViewSchedulingState(_, s) => s should contain(WriteTransformationCheckum(viewUnderTest))
      case _                                  => fail()
    }
  }
  ...
  it should "request writing of new transformation checksum if materialization mode is RESET_TRANSFORMATION_CHECKSUMS" in new NoOpLeafView {
    ...
    stateMachine.materialize(startState, dependentView, successFlagExists = false, materializationMode = RESET_TRANSFORMATION_CHECKSUMS) match {
      case ResultingViewSchedulingState(_, s) => s should contain(WriteTransformationCheckum(viewUnderTest))
      case _                                  => fail()
    }
  }
