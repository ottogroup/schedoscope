...
class ViewActorOld(view: View, settings: SchedoscopeSettings, hdfs: FileSystem, viewManagerActor: ActorRef, transformationManagerActor: ActorRef, metadataLoggerActor: ActorRef, var versionChecksum: String, var lastTransformationTimestamp: Long) extends Actor {
    ...
    case _: TransformationSuccess[_] => {
      ...
      if (view.transformation().isInstanceOf[FilesystemTransformation]) {
        if (viewDirectorySize > 0l) {
          ...
          listenersWaitingForMaterialize.foreach(s => s ! ViewHasNoData(view))
  ...
  def retrying(retries: Int, materializationMode: MaterializeViewMode): Receive = LoggingReceive({
    ...
    case Retry() => if (retries <= settings.retries)
      ...
      listenersWaitingForMaterialize.foreach(_ ! ViewFailed(view))
      listenersWaitingForMaterialize.clear()
    }
  })
  ...
  def failed: Receive = LoggingReceive({
    ...
    case MaterializeView(mode) => sender ! ViewFailed(view)
  })
  ...
  def aDependencyAnswered(dependency: org.schedoscope.dsl.View, mode: MaterializeViewMode) {
    ...
    if (oneDependencyReturnedData) {
      ...
      listenersWaitingForMaterialize.foreach(s => {
        log.debug(s"sending NoDataAvailable to ${s}"); s ! ViewHasNoData(view)
      })
      listenersWaitingForMaterialize.clear
